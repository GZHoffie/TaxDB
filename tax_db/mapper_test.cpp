...
int main() {
    ...
    std::filesystem::path genome_path("/home/zhenhao/data/taxonomy/DB_sampled_2.fa");
    ...
    std::string indicator("DB_sampled_2");

    // do the indexing 33147 19560
    auto indexer = new bucket_hash_indexer<24742>(bucket_len, read_len, shape, shape);
    indexer->index(genome_path, index_path, indicator);

    // perform the mapping
    auto mapper = new q_gram_mapper<24742>(bucket_len, read_len, segment_samples, shape, samples, fault, distinguishability);
